	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', extraDetail = '" . $product['extraDetail'] . "', extraDetail1 = '" . $product['extraDetail1'] . "', extraDetail2 = '" . $product['extraDetail2'] . "', extraDetail3 = '" . $product['extraDetail3'] . "', extraDetail4 = '" . $product['extraDetail4'] . "', extraDetail5 = '" . $product['extraDetail5'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");

				$order_product_id = $this->db->getLastId();

				foreach ($product['option'] as $option) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', extraDetail = '" . $product['extra'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "'");
				}
			}
		}
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
					...
					$data['products'][] = array(
						'name'     => $product['name'],
						'model'    => $product['model'],
						'extraDetail' => $product['extraDetail'],
                        'extraDetail1' => $product['extraDetail1'],
                        'extraDetail2' => $product['extraDetail2'],
