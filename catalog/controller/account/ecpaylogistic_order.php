...
if (isset($this->request->get['order_id'])) {
		...
		if ($ecpaylogistic_query->num_rows) {
			...
			if (strpos($aOrder_Info['shipping_code'],"ecpaylogistic.") !== false) {
				$ShowCVSMap = (($_SERVER['HTTPS']) ? "https" : "http") . '://' . $_SERVER['HTTP_HOST'] . str_replace('admin/','',$_SERVER['PHP_SELF']) . '?route=extension/shipping/ecpaylogistic/show_cvs_map_by_order&order_id='.$this->request->get['order_id'];
