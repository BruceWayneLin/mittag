...
if (strpos($order_info['shipping_code'],"ecpaylogistic.") !== false) {
	$CreateShippingOrder = $this->url->link('extension/shipping/ecpaylogistic/create_shipping_order&order_id='.$data['order_id'], 'token=' . $this->session->data['token'], 'SSL');
	...
	$ShowCVSMap = $this->url->link('extension/shipping/ecpaylogistic/show_cvs_map_by_order&order_id='.$data['order_id'], 'token=' . $this->session->data['token'], 'SSL');
